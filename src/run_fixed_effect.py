...
#time between sampling intervals, specific for fixed effect s
#note that we use the a heuristic expectation here (sigma^2 = 9 s^2). 
dt=int(min(0.5*N,L*rho/(3*s)**2+1.0/s))
...
def save_data():
    ...
    data_fname = '/ebio/ag-neher/share/users/rneher/RecAda/CoalescentRecombination/data_fixed_effect/hist_adaptive_'\
    +"_".join(map(str,['N', N,'L',L, 's', np.round(s,4),  'r', np.round(rho*L, 4)]))+id+ '.pickle'
...
################
fname = '/ebio/ag-neher/share/users/rneher/RecAda/CoalescentRecombination/seeds_fixed_effect/pop_'\
    +"_".join(map(str,['N', N,'L',L, 's', np.round(s,4),  'r', np.round(rho*L, 4)]))+'.pickle'
