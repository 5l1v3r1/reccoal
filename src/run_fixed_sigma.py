...
#time between sampling intervals, specific for fixed sigma
#note that we use the theoretical expectation here. 
dt=int(min(0.5*N,L*rho/sigma**2+5.0/sigma))
...
def save_data():
    ...
    data_fname = '/ebio/ag-neher/share/users/rneher/RecAda/CoalescentRecombination/data_fixed_sigma/hist_adaptive_'\
    +"_".join(map(str,['N', N,'L',L, 'sigma', np.round(sigma,4),  'r', np.round(rho*L, 4)]))+id+ '.pickle'
    ...
    with open(data_fname, 'w') as f:
        pickle.dump({'fithist': (fitnessbins, popfithist, ancestorfithist), 
                     ...
                     'selected_diversity': diversity_tmp, 
                     'TMRCA':np.asarray(TMRCA), 'T2':np.asarray(T2), 
                     'T3':np.asarray(T3) ,'T4':np.asarray(T4), 'blocks':np.asarray(pair_block),
...
for n in xrange(nsamples):
	...
	neutralSFS+=y
        ...
        for i in xrange(nT2):
            ...
            tmpT.calc_weight_distribution(tmpT.MRCA)
            tmpMRCA_weight = tmpT.nodes[tmpT.MRCA].weight_distribution
            tmpi=0
            tmpStep = tmpMRCA_weight[tmpi]
            while tmpStep.step>-0.5:
                tmpi+=1
                tmpStep = tmpMRCA_weight[tmpi]
            block_start = tmpMRCA_weight[tmpi-1].pos
            block_stop  = tmpMRCA_weight[tmpi].pos
            pair_block.append(block_stop-block_start)
...
################
fname = '/ebio/ag-neher/share/users/rneher/RecAda/CoalescentRecombination/seeds_fixed_sigma/pop_'\
    +"_".join(map(str,['N', N,'L',L, 'sigma', np.round(sigma,4),  'r', np.round(rho*L, 4)]))+'.pickle'
